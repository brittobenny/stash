...
export const downloadReceiptPdf = (order) => {
    ...
    const subtotal = items.reduce((acc, item) => acc + Number(item.price_each || 0) * Number(item.quantity || 0), 0);
  ...
  <g stroke='#f1c6c6' stroke-width='2' fill='none' stroke-linecap='round' stroke-linejoin='round' opacity='0.6'>
...
<head>
  <meta charset="utf-8" />
  <title>Stash Receipt #${order.id}</title>
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    th, td { text-align: left; padding: 8px; border-bottom: 1px solid #f0e6e0; font-size: 13px; }
...
<body>
  <div class="sheet">
    <div class="header">
      <div>
        <h1 class="title">Stash Receipt</h1>
        <div class="meta">Order #${order.id} · ${escapeHtml(orderDate)} · ${escapeHtml(order.status || '')}</div>
        <div class="badge">STASH MARKET</div>
      </div>
    ...
    <div class="grid">
      <div class="card">
        <h3>Customer</h3>
        <div>${escapeHtml(order.user_email || '')}</div>
        <div class="meta">${escapeHtml(order.user_phone || '')}</div>
      </div>
      <div class="card">
        <h3>Delivery</h3>
        <div>${escapeHtml(order.user_address || '')}</div>
        <div class="meta">${escapeHtml(order.user_location || '')}</div>
      </div>
    </div>
    ...
    <div class="footer">Thank you for shopping with Stash · Save as PDF from the print dialog</div>
  </div>
</body>
</html>`;
